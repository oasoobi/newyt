...
export function Video({ video }: { video: Video }) {
    return (
        <Link href={"/watch/" + video.videoId} className="flex border p-3 rounded-md hover:bg-gray-100 w-[80%] mb-5">
            <Image src={"/api/tn/" + video.videoId} width={250} height={100} alt="" className="rounded-md"/>
            <div className="ml-4">
                <h1>{video.title}</h1>
                <h1>{video.viewCountText}</h1>
                <h1>{video.publishedText}</h1>
                <div className="flex items-center">
                    <h1 className="mr-1">{video.author}</h1>
