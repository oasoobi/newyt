...
export function Playlist({ playlist }: { playlist: Playlist }) {
    return (
        <Link href={"/playlist/" + playlist.playlistId} className="flex border p-3 rounded-md hover:bg-gray-100 w-[80%] mb-5">
            <Image src={playlist.playlistThumbnail} width={250} height={100} alt="" className="rounded-md" />
            <div className="ml-4">
                <h1>{playlist.title}</h1>
                <h1>{playlist.videoCount} 本の動画</h1>
                <div className="flex items-center">
                    <h1 className="mr-1">{playlist.author}</h1>
